...
[p2_all,~,mu2_all] = polyfit(years,temps,2);
[p3_all,~,mu3_all] = polyfit(years,temps,3);

years_pred=2012:2050;
years_all=[years,years_pred];
years_pred_toplot=datetime(years_all,1,1);
ptemps1_pred = polyval(p1_all,years_all,[],mu1_all);
ptemps2_pred = polyval(p2_all,years_all,[],mu2_all);
ptemps3_pred = polyval(p3_all,years_all,[],mu3_all);
plot(years_pred_toplot,ptemps1_pred,'r--')
plot(years_pred_toplot,ptemps2_pred,'b--')
plot(years_pred_toplot,ptemps3_pred,'m--')
xlim([years_pred_toplot(1) years_pred_toplot(end)])
legend([f1,f2, f3],'1st-Degree','2nd-Degree','3rd-Degree','Location','NorthWest')
plot(yearstoplot, temps,'ok');
title('USA Average Temperature 1901-2012')
xlabel('Year')
